...
def Create(myID,envList,robotType,p,steps):
	...
	g['Min_Case_Error'] = [0]*16
	g['Max_Case_Error'] = [0]*16

	AAs = [0,2,8,10]
	BBs = [5,7,13,15]
	ABs = [1,3,4,6,9,11,12,14]
	...
	if (morphology == NM_MORPH):
		...
		#g['Min_Case_Error'] = [0.17, 0.1634, 0.1634,  0.287325]
		#g['Max_Case_Error'] = [1.0, 1.0, 1.0, 0.937223]


		for i in AAs:
			g['Min_Case_Error'][i] = .169
			g['Max_Case_Error'][i] = 1.0

		for j in BBs:
			g['Min_Case_Error'][j] = 0.287325
			g['Max_Case_Error'][j] = 0.937223

		for k in ABs:
			g['Min_Case_Error'][k] = .1634
			g['Max_Case_Error'][k] = 1.0
	#Create_Tree(g)
	...
	if (morphology == MOD_MORPH):
		...
		for i in AAs:
			g['Min_Case_Error'][i] = 0.0
			g['Max_Case_Error'][i] = 1.0

		for j in BBs:
			g['Min_Case_Error'][j] = 0.026
			g['Max_Case_Error'][j] = 1.0

		for k in ABs:
			g['Min_Case_Error'][k] = .013
			g['Max_Case_Error'][k] = 1.0
...
def Send_Tree_To_Simulator(g,env):
	...
	g['envError'][env] = abs(float(fitness)-g['Min_Case_Error'][env])/(g['Max_Case_Error'][env]-g['Min_Case_Error'][env])
	g['evaluated'][env] = True
